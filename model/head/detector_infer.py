...
class PostProcessor(nn.Module):
	...
	def tta_forward_1016(self, predictions, targets, features=None, test=False, refine_module=None, temp=None):
		pred_heatmap, pred_regression = predictions['cls'], predictions['reg']
		batch = pred_heatmap.shape[0]
		c, h, w = pred_heatmap.shape[1:]

		# max-pooling as nms for heat-map
		heatmap = nms_hm(pred_heatmap)
		tmp_heat_map = heatmap.view(batch, 3, -1)
		# Both in [N, C, K]
		topk_scores_all, topk_indices = torch.topk(tmp_heat_map, self.max_detection)
		score_dict = {}
		for i in range(batch):
			score_dict[i] = []
			cur_pred = topk_scores_all[i]  # [C, K]
			real_pred = cur_pred.t()  # [K, C]
			for j in range(self.max_detection):
				score_dict[i].append(real_pred[j])
		...
		for i in range(batch):
			tmp_feas = features[i]
			cur_row, cur_col = ys[i].int(), xs[i].int()
			for j in range(self.max_detection):
				selected_feas.append(tmp_feas[:, cur_row[j], cur_col[j]].unsqueeze(0))
		selected_feas = torch.cat(selected_feas, dim=0)
		return scores, score_dict, selected_feas

	def tta_forward_240525_update(self, predictions, targets, features=None, test=False, refine_module=None, detected_threshold=None, temp=None):
		tta_output = {}
		...
		for i in range(batch):
			...
			cur_pred = topk_scores_all[i]  # [C, K]
			# real_pred = cur_pred.t()  # [K, C]
			real_pred = cur_pred.transpose(0, 1)  # maintain the grad
			for j in range(self.max_detection):
				score_dict[i].append(real_pred[j])
		...
		tta_output['scores'] = scores
		tta_output['obj_xs'] = xs
		tta_output['obj_ys'] = ys
		tta_output['heatmap'] = heatmap
		tta_output['score_dict'] = score_dict
		
		pred_bbox_points = torch.cat([xs.view(-1, 1), ys.view(-1, 1)], dim=1)
		pred_regression_pois = select_point_of_interest(batch, indexs, pred_regression)

		tta_output['pred_regression_pois'] = pred_regression_pois

		# thresholding with score
		scores = scores.view(-1)
		if detected_threshold:
			indices = clses.view(-1).int().tolist()
			specific_thresholds = [detected_threshold[i] for i in indices]
			specific_thresholds = torch.tensor(specific_thresholds).cuda()
			valid_mask = scores >= specific_thresholds
		else:
			valid_mask = scores >= self.det_threshold

		tta_output['valid_mask'] = valid_mask
		...
		if valid_mask.sum() == 0:
			...
			return result, eval_utils, tta_output

		scores = scores[valid_mask]
		tta_output['all_clses'] = clses.view(-1).to(torch.int)
		clses = clses.view(-1)[valid_mask]
		tta_output['clses'] = clses.to(torch.int)
